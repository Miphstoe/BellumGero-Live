...
void CreatureManagerImplementation::droidHarvest(Creature* creature, CreatureObject* droid, int selectedID, int harvestBonus) {
	...
	int quantityExtracted = int(quantity * float(ownerSkill / 75.0f));
	// add in droid bonus
	...
	// Calculate droid bonus: harvestBonus stat directly adds percentage to harvested amount
	// Example: harvestBonus of 50 = 50% extra resources, 100 = 100% extra (double)
	int droidBonus = int(quantityExtracted * (harvestBonus / 100.0f));
...
void CreatureManagerImplementation::harvest(Creature* creature, CreatureObject* player, int selectedID) {
	...
	if (!player->isInRange(creature, 32)) //hardcoded harvest range
		return;
