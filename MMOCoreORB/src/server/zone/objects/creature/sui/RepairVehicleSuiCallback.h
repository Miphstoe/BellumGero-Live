...
public:
	...
	void run(CreatureObject* player, SuiBox* suiBox, uint32 eventIndex, Vector<UnicodeString>* args) {
		...
		// REMOVED: Disabled vehicle check - now allowing repairs on disabled vehicles
		...
		// Force update the vehicle's state to refresh display
		vehicle->setMaxCondition(vehicle->getMaxCondition());
		vehicle->notifyObservers(ObserverEventType::OBJECTINRANGEMOVED, vehicle);
